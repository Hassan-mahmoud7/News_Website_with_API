...
{{-- <style>
    .fixed-image {
    width: 200% !important; /* العرض المطلوب */
    height: 150% !important;/* الطول المطلوب*/
    object-fit: contain !important; /* عرض الصورة كاملة داخل الإطار */
}
</style> --}}
    <!-- Top News Start-->
    <div class="top-news">
        <div class="container">
            <div class="row">
                <div class="col-md-6 mt-0 tn-left">
                    <div class="row w-100 tn-slider">
                        @foreach ($latest_three_posts as $latest_post)
                        <div class="col-md-6 ">
                            <div class="tn-img">
                                <img class="img-fluid"src="{{ asset($latest_post->images->first()->path) }}" />
                                <div class="tn-title">
                                    <a href="{{ route('frontend.post.show',$latest_post->slug) }}" title="{{ $latest_post->title }}">{{ $latest_post->title }}</a>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="col-md-6 tn-right">
                    <div class="row">
                        ...
                        @endphp
                        @foreach ($four_posts as $four_post )
                        <div class="col-md-6">
                            <div class="tn-img" style="height: 164px;">
                                <img class="img-fluid w-100 h-100 " src="{{ asset($four_post->images->first()->path) }}" />
                                <div class="tn-title">
                                    <a href="{{ route('frontend.post.show',$four_post->slug) }}" title="{{ $four_post->title }}">{{ $four_post->title }}</a>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="tab-content">
                        ...
                        <div id="popular" class="container tab-pane fade">
                            ...
                            <div class="tn-news">
                                <div class="tn-img">
                                    <img src="{{ $post->images->first()->path ?? $post->images->first() }}" />
                                </div>
                                <div class="tn-title">
                                    <a href="{{ route('frontend.post.show',$post->slug) }}" title="{{ $post->title }}">{{ $post->title }} Comments({{  $post->comments_count}})</a>
                                </div>
                            </div>
                            @endforeach
                ...
                <div class="col-lg-9">
                    <div class="row">
                        @foreach ($all_posts as $post)
                            <div class="col-md-4">
                                <div class="mn-img">
                                    <img src="{{ $post->images->first()->path ?? $post->images->first()}}" loading="lazy"/>
                                    <div class="mn-title">
                                        <a  href="{{ route('frontend.post.show',$post->slug) }}" title="{{ $post->title }}">{{ $post->title }}</a>
                                    </div>
                                </div>
